...
<div class="col-lg-12 grid-margin stretch-card">
              <div class="card" style="width: 105%;">
                <div class="card-body">
                  <h3>Manage Subjects</h3><br>
                  <button type="button"  class="btn btn-primary" data-toggle="modal" data-target="#modalRelatedContent">New Subject</button>   <p class="card-description">
                    List of Subject
                  </p>
              ...
              <tbody>
               ...
               <tr>
                  ...
                  <td>{{ date('F d, Y g:ia', strtotime($subject->created_at )) }}</td>
                  <td>
    ...
    <div class="modal-dialog custom-modal-width">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-12">
                        <form action="/pdrrmo/insert-subject" method="post">
                            @csrf
                            <div class="form-group">
                                <label class="req-label">Sitrep Subject</label>
                                <input type="text" name="subject" class="form-control desc-long" placeholder="Enter subject" autocomplete="off" value="{{ old('subject') }}">
                                @error('subject')
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control desc-long" name="description" placeholder="Enter more detailed description " autocomplete="off">{{ old('description') }}</textarea>
                                @error('desc')
                                <div class="text-danger small">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="form-group">
                                <label class="req-label">ATTN</label>
                                <textarea class="form-control desc-long" name="attn" placeholder="Enter attn " autocomplete="off">{{ old('attn') }}</textarea>
                                @error('attn')
                                <div class="text-danger small">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="form-group">
                                <label class="req-label">FROM</label>
                                <textarea class="form-control desc-long" name="from" placeholder="Enter from" autocomplete="off">{{ old('from') }}</textarea>
                                @error('from')
                                <div class="text-danger small">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="form-group">
                                <label class="req-label">Disaster Preparedness</label>
                                <textarea class="form-control desc-long" name="disaster_preparedness" placeholder="Enter disaster preparedness " autocomplete="off">{{ old('description') }}</textarea>
                                @error('disaster_preparedness')
                                <div class="text-danger small">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="form-group">
                                <label class="req-label">Prepared by:</label>
                                <textarea class="form-control desc-long" name="prepared" placeholder="Enter name " autocomplete="off">{{ old('prepared') }}</textarea>
                                @error('desc')
                                <div class="text-danger small">{{ $message }}</div>
                                @enderror
                            </div>
